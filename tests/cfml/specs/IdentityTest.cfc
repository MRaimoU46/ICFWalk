...
component extends="icfwalktests.BaseSpec" output="false" {
	...
	public void function testHeaderProviderTrustsOnlyListedProxies() {
		...
		assertExactTextEquals("sso|abc123", ok.subject);
		assertExactTextEquals("Fixture Person", ok.displayName);
		assertExactTextEquals("fixture@example.org", ok.email);
		...
		assertExactTextEquals("untrusted_source", spoof.reason);
	...
	public void function testHeaderProviderRejectsMissingOrMalformedSubjects() {
		...
		assertExactTextEquals("", noEmail.email);
	}
	...
	public void function testDevelopmentStubReadsOnlyItsOwnHeaders() {
		...
		assertTrue(ok.authenticated);
		assertExactTextEquals("dev-user-1", ok.subject);
	...
	public void function testConfigLoaderRejectsUnsafeIdentitySettings() {
		...
		assertExactTextEquals("header", ok.ssoMode);
	...
	public void function testProvisioningIsKeyedBySubjectAndTolerantOfEmailCollisions() {
		...
		assertExactTextEquals(variables.fx.tag() & "-prov-a", a.subject);
		assertTrue(a.active);
		var again = users.findBySubject(a.subject);
		assertExactTextEquals(a.userId, again.userId, "Lookup by subject returns the same account.");
		...
		assertExactTextEquals("", b.email, "Colliding email is not stored twice.");
		variables.c.db.run("DELETE FROM [icf].[app_user] WHERE user_id = :id", { "id": variables.c.db.guid(b.userId) });
		assertExactTextEquals("Renamed Fixture", users.findById(a.userId).displayName);
	}
	...
	public void function testAuthenticationProvisionsAndBuildsPrincipal() {
		...
		assertExactTextEquals(subject, principal.subject);

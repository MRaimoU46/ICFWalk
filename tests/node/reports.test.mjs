...
// CSV download contract, and injection payloads in every parameter (SEC-01). Fixtures are created
// through the maintenance endpoints and removed afterwards. With ICFWALK_REQUIRE_APP=1 an
// unreachable application fails the run rather than skipping it.
...
/** Create, fill and complete a walk as `walker` at their school. */
async function completedWalk(walker, rating, extra = {}) {
  ...
  const saved = await walker.call("PUT", `/api/walks/${w.id}`, {
    ...
    dimensions: { grade: { selectedValueCode: "7" }, observer: { textValue: SENTINELS.observer }, content: { selectedValueCode: "other", otherText: SENTINELS.other }, ...(extra.dimensions || {}) },
    responses: {
...
test("RPT-01: a school report role gets its school; another school is 404 over HTTP", { skip }, async () => {
  const own = await who.schoolReport.call("GET", "/api/reports/aggregate");
  assert.equal(own.status, 200, own.text);
  assert.equal(own.json.population.walks, 2);
  ...
  for (const other of [units.b, units.c, units.district]) {
    for (const route of ["/api/reports/aggregate", "/api/reports/aggregate.csv"]) {
      const r = await who.schoolReport.call("GET", `${route}?orgUnitId=${other}`);
      assert.equal(r.status, 404, `${route} ${other}`);
...
test("RPT-02: a district report role aggregates its descendant schools and nothing else", { skip }, async () => {
  const r = await who.districtReport.call("GET", "/api/reports/aggregate");
  assert.equal(r.status, 200, r.text);
  assert.equal(r.json.population.walks, 3);
  ...
  assert.deepEqual(rating.options.map((o) => [o.code, o.count]), [["1", 0], ["2", 1], ["3", 0], ["4", 1], ["5", 1]], "school C's rating of 1 is not in scope");
  assert.equal(rating.scored.responses, 3);
  assert.equal(rating.scored.mean, 3.6667);
  assert.equal((await who.districtReport.call("GET", `/api/reports/aggregate?orgUnitId=${units.b}`)).json.population.walks, 1);
  assert.equal((await who.districtReport.call("GET", `/api/reports/aggregate?orgUnitId=${units.c}`)).status, 404);
});

test("RPT-03 / RPT-06: payload and export carry no walk, owner, narrative, email or free text", { skip }, async () => {
  const json = await who.schoolReport.call("GET", "/api/reports/aggregate");
  const csv = await who.schoolReport.call("GET", "/api/reports/aggregate.csv");
  assert.equal(csv.status, 200);
  for (const text of [json.text, csv.text]) {
  ...
  const content = json.json.dimensions.find((d) => d.code === "content");
  assert.equal(content.values.find((v) => v.code === "other").walks, 2);
...
test("the CSV export is an attachment with a safe name, no-store, nosniff, a BOM and CRLF rows", { skip }, async () => {
  const r = await who.districtReport.call("GET", `/api/reports/aggregate.csv?orgUnitId=${units.a}`);
  assert.equal(r.status, 200, r.text);
  ...
  assert.equal(lines[0], "record_type,group,key,label,count,answered,unanswered,hidden,not_applicable,unrecorded,scored_responses,score_sum,mean,suppressed");
  assert.ok(lines.every((l) => !l.includes("\n")), "no bare line feed");
  assert.ok(lines.includes("POPULATION,,walks,,2,,,,,,,,,0"), "the population row");
  assert.ok(lines.some((l) => l.startsWith("ITEM,s1,comp_s1_q1,")), "item rows");
  assert.ok(lines.includes("OPTION,comp_s1_q1,4,4,1,,,,,,,,,0"));
  const agg = await who.districtReport.call("GET", `/api/reports/aggregate?orgUnitId=${units.a}`);
...
test("SEC-01: injection payloads in any parameter are refused as data, and nothing is returned", { skip }, async () => {
  ...
  for (const [name, value, code] of payloads) {
    for (const route of ["/api/reports/aggregate", "/api/reports/aggregate.csv"]) {
      const r = await who.districtReport.call("GET", `${route}?${new URLSearchParams([[name, value]])}`);
  ...
  // The data is still there afterwards.
  assert.equal((await who.districtReport.call("GET", "/api/reports/aggregate")).json.population.walks, 3);
});

test("options describe the version, the scope and the reportable surface only", { skip }, async () => {
  const r = await who.districtReport.call("GET", "/api/reports/options");
  assert.equal(r.status, 200, r.text);
  assert.equal(r.json.format, "icfwalk-aggregate-report/1");

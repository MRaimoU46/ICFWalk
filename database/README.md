...
7. Apply `007_report_release.sql` (Phase 7 correction, RPT-03). Additive and safe to re-run. It creates the frozen report releases report-only users read -- `icf.report_release` (one row per release: its dates `observed_from`..`observed_to`, the minimum it was made with, who released it and when, and the transaction that created it), `icf.report_release_block` (one row per (release, version, org unit) block that met the minimum), `icf.report_release_cell` (the non-zero category counts of each reportable breakdown of each block, keyed by instrument codes) and `icf.report_release_walk` (the walks each stored block counts, keyed by the walk: a walk belongs to one release at most; no report reads it) -- and the guards the database enforces whatever writes the rows: `TR_report_release_no_overlap` (no two releases cover the same date), `TR_report_release_block_floor` and `CK_report_release_block_walks` (no block below its release's minimum, nor below 3), `CK_report_release_minimum` (no release below 3), `PK_report_release_walk` (no walk in a second release), `TR_report_release_block_members` and `TR_report_release_walk_guard` (a block counts exactly its recorded walks, and they never change: 50065), the `*_sealed` checks in those triggers and `TR_report_release_cell_sealed` (nothing is added to a release after the transaction that created it: 50066), the immutability triggers (nothing is ever updated: 50064), and `TR_report_release_no_delete`, `TR_report_release_block_no_delete`, `TR_report_release_cell_no_delete`, `TR_report_release_walk_no_delete` (nothing is ever deleted, in whole or in part: 50068). A database that applied an earlier, unreleased draft of this patch (a `report_release` table without `created_transaction_id`) is refused with 50067 rather than altered. Nothing in the application deletes a release. Removing one takes a principal allowed to alter the schema, which can switch the delete guards off; the runtime login must never be one (below), and a removed release frees its dates and its walks for a second release that could be combined with it (`../docs/DATA_CONTRACT.md`, "Aggregate privacy rule (RPT-03)").
...
- Use a least-privilege database login for the application: data permissions (`SELECT`, `INSERT`, `UPDATE`, `DELETE`) on the `icf` schema, and never `ALTER`, `CONTROL`, `db_owner` or `db_ddladmin`. The report release guards of `007_report_release.sql` refuse every deletion of a release row to such a login; a login that may alter the schema could switch them off or truncate the tables. Apply migrations with a separate, privileged login.

...
(chunked) is read from the servlet container's input stream at most one byte past the limit, counted
in bytes.
...
- **If ColdFusion reads the body before the application does.** The application reads the
  ...
  body in UTF-8 bytes before parsing it. Then the settings above are what bounds the engine's own
  buffering of a chunked body, which is why they must be set.
- **Lucee (the verification runtime below).** Lucee's request wrapper copies the whole body into
  ...
  the limits, and they are what `RouterBodyOrderTest` and `tests/node/admin-instrument.test.mjs`
  verify.

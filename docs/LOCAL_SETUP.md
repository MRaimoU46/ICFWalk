...
The application bounds every request body itself (P6A-01; `docs/ENDPOINTS.md`, "Request bodies"):
after the caller is authenticated and the CSRF token checked, and before anything is parsed, an
instrument import may be at most 5,000,000 bytes and any other request at most 20,000,000 bytes. A
declared `Content-Length` over the limit is refused without reading the body; a body without one
(chunked) is read from the servlet container's input stream in requests of at most
`min(65,536, limit - bytes read + 1)` bytes, so the application takes at most one byte past the limit
from it, counted in bytes (P6A-R01). That bounds what the application consumes, not what the
container has already received from the network into its own buffers -- which is one reason the
connector limit below matters.

Set the same ceiling at the server, so a body the application would refuse is never buffered by
anything in front of it either:

- **Adobe ColdFusion 2023 (production).** ColdFusion Administrator, Server Settings > Settings >
  Request Size Limits: **Maximum size of post data = 20 MB** (the default). Keep it at least 20 MB, or
  import documents near the 5 MB limit and large walk saves are refused by ColdFusion before the
  application can answer.
- **IIS in front of ColdFusion.** Request Filtering: `<requestLimits maxAllowedContentLength="20000000" />`
  under `system.webServer/security/requestFiltering` in the site's `web.config` (IIS's default is
  30,000,000 bytes). Apache: `LimitRequestBody 20000000`.
- **If ColdFusion reads the body before the application does.** The application reads the
  container's stream through ColdFusion's request wrapper (`ServletRequestWrapper.getRequest()`);
  if that stream is already consumed, it falls back to `getHttpRequestData(true)` and measures the
  body in UTF-8 bytes before parsing it. That copy is measured, not bounded: the settings above are
  then what bounds the engine's own buffering of a chunked body, which is why they must be set.
- **Lucee (the verification runtime below).** Lucee's request wrapper copies the whole body into
  memory the first time its input stream is read, so the application reads the container's stream
  underneath it (`getOriginalRequest()`), which is how the bounded read is enforced there. The
  runtime configures no connector limit of its own: `jetty-runner` 9.4 has none for non-form
  bodies (`maxFormContentSize` applies to form posts only), so on Lucee the application's limits are
  the limits, and they are what `RouterBodyOrderTest`, `RequestBodyReadBoundTest` (the production
  reader's exact bound) and `tests/node/admin-instrument.test.mjs` verify.

None of the ColdFusion, IIS or Apache settings above has been exercised in this repository's
verification: they are the documented equivalents, to be confirmed on the target platform.
...
| `npm run test:admin` | Phase 6: `tests/node/admin-publish.test.mjs` (the publish route), `tests/node/admin-instrument.test.mjs` (HTTP checks of every administration route: 401/403/CSRF on each, import with its validation summary, the body contracts and the 413 cap, preview equal to the runtime model, clone + edit + compare, stale-edit refusal, retire with a historical walk that still opens and saves, the only-version confirmation, the placeholder queue, discard) and `tests/node/browser-admin.test.mjs` (Playwright: an admin-only user lands on administration, file upload and the summary, preview with conditional behavior and no writes, new draft + prompt edit + compare with markup kept as text, publish and retire confirmations, the placeholder queue, keyboard, axe-core and 375/768 px; screenshots `admin-*.png`). Fixtures use their own instrument codes and never publish or retire `ICFWALK`. Since the audit corrections, also: the request-body order and limits (401/403 before any body is read, 413 before parsing, UTF-8 bytes for chunked bodies -- proved with bodies that never finish), the create-only/replace import contract, and in the browser a stale page that cannot overwrite a newer draft, a late-created label that is never silently replaced, oversized files refused before a full read, and rejected workbooks that send nothing. The CFML twins run inside `npm run test:cfml` (`InstrumentAdministrationTest`, `DraftEditorTest`, `InstrumentVersionComparerTest`, `RetireConcurrencyBarrierTest`, and the audit corrections' `RouterBodyOrderTest`, `RequestBodyReadBoundTest`, `DraftReplacementConcurrencyTest`, `DiscardIdentityBarrierTest`). |
| `npm run test:workbook` | The Excel round-trip's converter (`tests/node/workbook.test.mjs`): the aligned workbook in `config/`, LibreOffice-saved fixtures in `tests/fixtures/workbooks`, round trips, value handling, problems by cell, hostile files (including out-of-range, surrogate and forbidden character references and malformed cell references, each a structured problem rather than an exception), and locating server problems. Needs nothing running. |
